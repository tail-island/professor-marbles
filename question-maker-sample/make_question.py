...
# ルール上は試験管の数の最大値は32、試験管のサイズの最大値は32、ビーダマの色は255ですけど、探索が終わらなそうなので制限してみました。
...
def get_legal_actions(test_tubes):
    for i in filter(lambda i: len(test_tubes[i]) > 0, range(test_tube_count)):  # 移動先でループします。ビーダマが入っている試験管は、他の試験管からビーダマが移動してきた可能性があります。
        for j in filter(lambda j: len(test_tubes[j]) < test_tube_sizes[j] and j != i, range(test_tube_count)):  # 移動元でループします。ビーダマが満タンではない試験管は、他の試験管にビーダマを移動させられた可能性があります。
...
farthest_node = answer_test_tubes
farthest_node_path_length = 0

for node, node_path_length in nx.shortest_path_length(graph, source=answer_test_tubes).items():
    if node_path_length > farthest_node_path_length:
        farthest_node = node
        farthest_node_path_length = node_path_length

print(farthest_node, file=sys.stderr)
print(farthest_node_path_length, file=sys.stderr)
...
for test_tube in farthest_node:
    print(' '.join(map(str, test_tube)))
